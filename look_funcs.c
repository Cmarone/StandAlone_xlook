...
/*------------------------------------------------------------------------*/
/*changes to formats, cjm 20150517*/
void review(FILE *file, int *firstcol, int *lastcol)
	//FILE *file ;
...
//fprintf(stderr,"in here... number of records = %d\n     ",head.nrec) ;

	fprintf(file,"%s",data_file) ;
	for( i = *firstcol; i <= *lastcol; ++i )
	{
		if ( strncmp(&(head.ch[i].name[0]),"no_val",6) != 0)
		{
	...
	//FILE *file ;
{
	...
	for ( i = *firstrow; i <= *lastrow ; ++i)
	{
		...
		for ( j = *firstcol; j <= *lastcol; ++j)
		{
			...
			{
				fprintf(file,", %g",darray[j][i]) ;   //do we need more precision than this?
			}
		}
		fprintf(file,"\n") ;
	}
	...
	//     int *firstrow , *lastrow , *firstcol , *lastcol ;
{
	...
	for ( i = *firstrow; i <= *lastrow ; ++i)
	{
		...
		for ( j = *firstcol; j <= *lastcol; ++j)
		{
			if(strncmp(&(head.ch[j].name[0]),"no_val",5) != 0)
			{
				sprintf(tmp_msg, "  %10.10g",darray[j][i]);
				strcat(msg, tmp_msg);
			}
		}
		strcat(msg, "\n");
...
int name(int *unknown, int orig_col, char *arg_name, char *arg_unit)		/*returns 0 on error, 1 if successful*/
     //int *unknown;
     //int orig_col;
