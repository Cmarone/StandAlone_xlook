...
/*************************** polyfit ************************/
void do_polyfit(char arg[])
{
  double *px, *py, *pw, *pjm1, *pj, *error, ortval(double x);
  ...
  if(desire[(strlen((char *)desire)-1)] == 'r')
    {
      ...
      /* call doit_proc(); set ui_globals.action to MAIN first so that it will not go back to interpolater if doit fails */
    //ui_globals.action = MAIN;
...
/*************************** r_trend ************************/
void do_r_trend_input(char arg[])
{
  nocom(arg);
...
//ui_globals.action = MAIN;
  ...
  */
  ...
  /* set simp_func_ui_globals.action so that it knows from where it's called from
     set ui_globals.action to simp func
     */

  simp_func_action = SIMPLEX;
//ui_globals.action = SIMP_FUNC;
  return;
...
void do_median_smooth(char arg[])
{
  nocom(arg);
  ...
  if ((strncmp(new_file,"S",1) == 0 || strncmp(new_file,"s",1) == 0) && 
  ...
  else
    {
      ...
      if ((new = fopen(new_file, "w")) == NULL )
        {
	  sprintf(msg, "Can't open file %s.\n", new_file);
fprintf(stderr,"error from cmds.c do_type_final() %s\n",msg);
	  print_msg(msg);
	  top();
	//ui_globals.action = MAIN;
	  return;
        }
	  /*cjm 20240306 this was the method to ask if you wanted to overwrite an existing file.... I axed it for StandAlone...*/

/*      if (strncmp(desire,"type_p",6) == 0)
	p_vision(new,&i,&j,&h,&k) ;
      else
	vision(new,&i,&j,&h,&k);  cjm 20240306 just use new type_p version...*/
...
void do_scm(char arg[])
{
  ...
  if(l==2)
    ...
    //ui_globals.action = SCM_SIMP_WEIGHT_L2;
      return;
    }
  else
    {
      ...
      set_left_footer("Type the arguments for simplex weight");
      set_cmd_prompt("Input: ");
    //ui_globals.action = SCM_SIMP_WEIGHT;  
      return;
    }
}


void do_simp_weight_l2(char arg[])
{
  /*     int l, *temp_int; */
  ...
  *disp_ptr = darray[rs_param.disp_col][rs_param.last_row]+1e9 ;
  ...
  if(rs_param.a < 0)		/*calculate error matrix*/
  ...
  else	/* just do the simp calc like I used to */
    {
      ...
      /* 
	 set simp_func_ui_globals.action so it knows where to go to after that
