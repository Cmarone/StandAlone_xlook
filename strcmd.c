     ...
     int num)
{
	...
	char buf[8];
	char *strtok();
	int token_count(char *buf, int maxxlength);
	int maxxlength;
	...
	else		/*ok, go ahead*/
	{
		...
		if(token_count(str, maxxlength) <= 2)	/*try to avoid the problem of user changing their mind and just typing one letter*/
			return(-1);

		/*fprintf(stderr,"colnum=%d in getcol %s, len=%d\n", colnum, str, (int)strlen(str));
		fprintf(stderr,"tokens=%d, in getcol %s, len=%d\n", token_count(str, maxxlength),str, (int)strlen(str));*/
